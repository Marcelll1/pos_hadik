...
static int cell_is_obstacle(const game_state_t *g, uint8_t x, uint8_t y) {
    ...
    return g->obstacle_map[y * g->map_width + x] ? 1 : 0;
}
...
int game_load_map_from_file(game_state_t *g, const char *path) {
    ...
    for (uint8_t y = 0; y < g->map_height; y++) {
        ...
        for (uint8_t x = 0; x < g->map_width; x++) {
            char c = line[x];
            g->obstacle_map[y * g->map_width + x] = (c == '#') ? 1 : 0;
        }
    }
...
static int spawn_is_safe(const game_state_t *g, game_pos_t head, direction_t start_dir) {
    ...
    game_pos_t p1 = step_in_world(g, head, start_dir);
    game_pos_t p2 = step_in_world(g, p1, start_dir);
    game_pos_t p3 = step_in_world(g, p2, start_dir);

    if (!cell_is_safe_for_spawn_path(g, p1)) return 0;
    if (!cell_is_safe_for_spawn_path(g, p2)) return 0;
    if (!cell_is_safe_for_spawn_path(g, p3)) return 0;
...
int game_join_new_player(game_state_t *g, int player_slot, const char *player_name, uint64_t now_ms) {
    ...
    if (g->start_time_ms == 0) {
        g->start_time_ms = now_ms;
        if (g->game_mode == GAME_MODE_TIMED && g->timed_end_ms == 0) {
            g->timed_end_ms = now_ms + 60000ULL;
        }
    }
...
void game_handle_leave(game_state_t *g, int player_slot) {
    if (player_slot < 0 || player_slot >= GAME_MAX_PLAYERS) return;
...
void game_tick(game_state_t *g, uint64_t now_ms) {
    ...
    for (int s = 0; s < GAME_MAX_PLAYERS; s++) {
        ...
        int will_grow = is_food_at(g, new_head, &eaten_food_index);
        ...
        if (will_grow) {
            ...
            pl->score++;
            remove_food_at(g, eaten_food_index);
        }
...
static char head_char(int slot) {
...
static char body_char(int slot) {
    if (slot >= 0 && slot < 26) return (char)('a' + slot);
    slot -= 26;
    if (slot >= 0 && slot < 10) return (char)('0' + slot);
    return '+';
...
void game_build_ascii_map(const game_state_t *g, uint8_t *out_cells) {
    for (int i = 0; i < STATE_MAP_CELLS; i++) out_cells[i] = (uint8_t)' ';

    if (g->world_type == WORLD_FILE) {
        for (uint8_t y = 0; y < g->map_height; y++) {
            for (uint8_t x = 0; x < g->map_width; x++) {
                if (cell_is_obstacle(g, x, y)) {
                    out_cells[y * g->map_width + x] = (uint8_t)'#';
                }
            }
        }
    }
    ...
    for (uint8_t i = 0; i < g->food_count; i++) {
        ...
        out_cells[f.y * g->map_width + f.x] = (uint8_t)'*';
    }
    ...
    for (int s = 0; s < GAME_MAX_PLAYERS; s++) {
        ...
        if (!pl->is_alive && pl->snake_len > 0) {
            for (uint16_t i = 0; i < pl->snake_len; i++) {
                ...
                out_cells[p.y * g->map_width + p.x] = (uint8_t)'x';
            }
            continue;
        }
        ...
        if (pl->snake_len > 0) {
            ...
            out_cells[h.y * g->map_width + h.x] = (uint8_t)head_char(s);
        }
        for (uint16_t i = 1; i < pl->snake_len; i++) {
            game_pos_t p = pl->snake_body[i];
            out_cells[p.y * g->map_width + p.x] = (uint8_t)body_char(s);
        }
    }
}
